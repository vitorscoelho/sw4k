...
//    val bByRef = DoubleArrayByRef(size = 4)
//    val dByRef = DoubleArrayByRef(size = 6)
...
//    sapModel.analyze.runAnalysis()
    ...
    val m3ByRef = DoubleArrayByRef(size = 1)
    println("Resultado função FrameForce: ${sapModel.results.frameForce(
            name = "HS",
            itemTypeElm = ItemTypeElm.OBJECT_ELM.sapId,
            numberResults = numberResults,
            obj = StringArrayByRef(size = 1),
            objSta = DoubleArrayByRef(size = 1),
            elm = StringArrayByRef(size = 1),
            elmSta = DoubleArrayByRef(size = 1),
            loadCase = StringArrayByRef(size = 1),
            stepType = StringArrayByRef(size = 1),
            stepNum = DoubleArrayByRef(size = 1),
            p = DoubleArrayByRef(size = 1),
            v2 = DoubleArrayByRef(size = 1),
            v3 = DoubleArrayByRef(size = 1),
            t = DoubleArrayByRef(size = 1),
            m2 = DoubleArrayByRef(size = 1),
            m3 = m3ByRef
    )}")
    println("Qtd. resultados= ${numberResults.value}")
    println("momento= ${m3ByRef.size}")
